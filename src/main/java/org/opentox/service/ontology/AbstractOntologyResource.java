...
public abstract class AbstractOntologyResource extends ServerResource implements
		IAuthToken {
	...
	abstract protected Dataset createOntologyModel(boolean init)
	...
	private static String header = "<div class='row half-bottom' id='header'>\n"
	...
	enum Keys {
		 ...
		 * FeatureValue {
		 * 
		 * @Override public Keys parent() { return DataEntry; } }, DataEntry {
		 * 
		 * @Override public Keys parent() { return Dataset; } },
		 */
		Dataset {
			@Override
			public String getSPARQL() {
		...
		ToxCast_CYP450 {
			@Override
			public String getSPARQL() {
				return String.format("%s%s", getPrefix(), String.format(
						sparql_ToxcastAssayTarget, "Cytochrome_P450"));
		...
		ToxCast_Ion_Channel {
			...
			public String getSPARQL() {
				return String
						.format("%s%s", getPrefix(), String.format(
								sparql_ToxcastAssayTarget, "Ion_Channel"));
		 ...
		 * 
		 * @Override public String getSPARQL() { return
		 * String.format("%s%s",getPrefix(), String.format(
		 * sparql_ToxcastAssayTarget,"Membrane_Protein") );
		 * 
		 * }
		 * 
		 * @Override public Keys parent() { return ToxCast; } },
		 * ToxCast_Nuclear_Receptor {
		 * 
		 * @Override public String getSPARQL() { return
		 * String.format("%s%s",getPrefix(), String.format(
		 * sparql_ToxcastAssayTarget,"Nuclear_Receptor") );
		 * 
		 * }
		 * 
		 * @Override public Keys parent() { return ToxCast; } },
		...
		/*
		 * Anti-Coagulation Apolipoprotein Cell_Adhesion_Molecule
		 * Coagulation_Factor Cytokine Cytokine_Receptor Hormone Lyase Oxidase
		 * Oxygenase Other Phosphatase Protease Reductase Sigma_Receptor
		 * Synthase Transferase Transporter
		 */
		/*
		 ...
		 * @Override public String getSPARQL() { return
		 * String.format("%s%s",getPrefix(), String.format(
		 * sparql_ToxcastAssayTarget,"Cytokine_Receptor") );
		 * 
		 * }
		 * 
		 * @Override public Keys parent() { return ToxCast; } },
		 ...
		 * @Override public String getSPARQL() { return
		 * String.format("%s%s",getPrefix(), String.format(
		 * sparql_ToxcastAssayTarget,"Esterase") );
		 * 
		 * }
		 * 
		 * @Override public Keys parent() { return ToxCast; } };
		...
		public String getSPARQL(String nameSpace, String predicate) {
			...
			return String.format("%s\n" + "	select ?%s ?title\n" + "	where {\n"
					...
					getPrefix(), name(), name(), predicate, nameSpace, name(),
					name());
		}
	...
	protected void doInit() throws ResourceException {
		...
		customizeVariants(new MediaType[] { MediaType.TEXT_HTML,
				...
				MediaType.APPLICATION_SPARQL_RESULTS_JSON, MediaType.TEXT_CSV,
				MediaType.TEXT_PLAIN, MediaType.TEXT_URI_LIST });
		try {
			resultsOnly = getRequest().getResourceRef().getQueryAsForm()
					.getFirstValue("results").equals("yes");
		} catch (Exception x) {
			resultsOnly = false;
		}
		try {
			title = getRequest().getResourceRef().getQueryAsForm()
					.getFirstValue("title");
		} catch (Exception x) {
			title = null;
		}
	...
	protected Representation get(Variant variant) throws ResourceException {
		...
		if (query == null) {
			if (uri != null) {
				query = String
						.format("PREFIX ot:<http://www.opentox.org/api/1.1#>\n"
								+ "PREFIX ota:<http://www.opentox.org/algorithmTypes.owl#>\n"
								+ "PREFIX owl:<http://www.w3.org/2002/07/owl#>\n"
								+ "PREFIX dc:<http://purl.org/dc/elements/1.1/>\n"
								+ "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n"
				...
				if (key == null) {
					query = "PREFIX ot:<http://www.opentox.org/api/1.1#>\n"
							+ "PREFIX ota:<http://www.opentox.org/algorithmTypes.owl#>\n"
							+ "PREFIX owl:<http://www.w3.org/2002/07/owl#>\n"
							+ "PREFIX dc:<http://purl.org/dc/elements/1.1/>\n"
							+ "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n"
					...
					try {
						...
						throw new ResourceException(
								Status.CLIENT_ERROR_BAD_REQUEST, key.toString());
					}
			}
		}
	...
	protected void readOntologies(Model ontology) {
		String[] owls = new String[] { "opentox.owl",
				"bibtex-converted/descriptor-algorithms.owl",
				"echa-endpoints.owl", "AlgorithmTypes.owl",
				"bibtex-converted/descriptors-ambit.owl",
				"bibtex-converted/ist-algorithms.owl", "toxcast.owl",
				"bibtex.owl" };
		for (String owl : owls)
			try {
				readOWL(getClass().getClassLoader().getResourceAsStream(
						String.format("org/opentox/owl/%s", owl)), ontology);
			} catch (Exception x) {
	...
	protected Model getOntology(Model model, Reference reference)
			throws ResourceException {
		try {

			MediaType[] mt = { MediaType.APPLICATION_RDF_XML,
			...
			for (MediaType m : mt) {
				InputStream in = null;
				try {
					cli = ClientResourceWrapper.getHttpURLConnection(
							reference.toString(), "GET", m.toString());
					cli.connect();
					if (HttpURLConnection.HTTP_OK == cli.getResponseCode()) {
						in = cli.getInputStream();
						tmpFile = File.createTempFile("ontologyservice_",
								".rdf");
						...
						throw new ResourceException(
								Status.SERVER_ERROR_BAD_GATEWAY, String.format(
										"%d %s %s", cli.getResponseCode(),
										cli.getResponseMessage(),
										reference.toString()));
				} catch (ResourceException x) {
					throw new ResourceException(
							Status.SERVER_ERROR_BAD_GATEWAY, String.format(
									"%s %s %s",
									Status.SERVER_ERROR_BAD_GATEWAY.toString(),
									reference, x.getMessage()), x);
				} catch (IOException x) {
					throw new ResourceException(Status.SERVER_ERROR_INTERNAL,
							String.format("%s %s %s",
									Status.SERVER_ERROR_INTERNAL.toString(),
									reference, x.getMessage()), x);
				} catch (Exception x) {
					throw new ResourceException(
							Status.CLIENT_ERROR_BAD_REQUEST,
							reference.toString(), x);
				} finally {
					try {
						if (in != null)
							in.close();
	...
	protected Representation sparql(final String queryString, Variant variant)
			throws ResourceException {
		...
		return new OutputRepresentation(variant.getMediaType()) {
			...
			public void write(OutputStream out) throws IOException {
				...
				try {
					...
					try {
						Query query = QueryFactory.create(queryString, null,
								Syntax.syntaxARQ);
						...
						if (getMediaType()
								.equals(MediaType.APPLICATION_RDF_XML))
							ResultSetFormatter.outputAsRDF(out, "RDF/XML",
									results);
							...
							for (String style : css)
								w.write(String.format(style, getRequest()
										.getRootRef()));
							// js
							for (String script : js)
								w.write(String.format(script, getRequest()
										.getRootRef()));
							...
							w.write(String
									.format("<script>$(function() {$( \".datatable\" ).dataTable({%s });});</script>",
											dtableOptions));
							...
							if (!resultsOnly) {
								w.write(String.format(header, getRequest()
										.getRootRef()));
								w.write("<FORM>");
								writehtmlheader(w, ontology, queryString,
										elapsed);
							} else
							...
							while (results.hasNext()) {
								...
								for (int i = 0; i < vars.size(); i++) {
									...
									w.write("<td>");
									if (node != null) {
										String value = node.isLiteral() ? ((Literal) node)
												.getString() : PrintUtil
												.print(node);
										if ((value.indexOf("<") >= 0)
												&& (value.indexOf(">") >= 0))// some
																				// xml
																				// inside
											w.write(String
													.format("<textarea readonly width='100%%'>%s</textarea>",
															value));
										else if (value.startsWith("http"))
											w.write(String
													.format("%s&nbsp;<a href='?uri=%s'>?</a>",
															value,
															Reference
																	.encode(value)));
							...
							if (!resultsOnly) {
								w.write("</FORM>");
								w.write(String
										.format("<div class='row'><div class='columns ten'>Results [found in %d ms]</div></div>",
												elapsed));
							}

							w.write(jsGoogleAnalytics() == null ? ""
									: jsGoogleAnalytics());
	...
	public static String jsGoogleAnalytics() {
		if (jsGoogleAnalytics == null)
			try {
				BufferedReader reader = new BufferedReader(
						new InputStreamReader(
								TDBOntologyResource.class
										.getClassLoader()
										.getResourceAsStream(
												"org/opentox/config/googleanalytics.js")));
	...
	protected void removeURI(Model ontology, String uri) throws Exception {
		StmtIterator iter = ontology.listStatements(new SimpleSelector(ontology
				.createResource(uri), null, (RDFNode) null));
		ontology.remove(iter);
	}

	public void writehtmlheader(Writer w, Model ontology, String queryString,
			long elapsed) throws Exception {
		w.write("<div class='ui-widget-header row remove-bottom' style='margin-left:5px;margin-right:5px;'>");

		w.write(String
				.format("<a class='remove-bottom' href='%s/ui' title='Home'>Home</a>&nbsp;",
						getRequest().getRootRef()));

		Keys qkey = null;
		try {
			qkey = Keys.valueOf(getRequest().getAttributes().get(resourceKey)
					.toString());
		} catch (Exception x) {
		}
		StringBuilder b = new StringBuilder();
		...
		for (Keys key : Keys.values()) {
			...
			String sparql = String
					.format("<a class='remove-bottom' href='%s%s/%s' title='%s\n\n%s'>%s</a>&nbsp;",
							getRequest().getRootRef(), tag, key.name(),
							key.getHint(), key.getSPARQL(), key.toString()
		...
		w.write(String
				.format("<div class='row ui-widget-header ui-corner-top remove-bottom'  style='margin-left:5px;margin-right:5px;'>Search the Ontology service&nbsp;[%s triples]</div>"
						...
						ontology == null ? 0 : ontology.size(), getRequest()
								.getRootRef()));
		w.flush();
		w.write(queryString);
		w.write("</TEXTAREA>"
